...
(function($) {
	function autosuggest(callBackUrl, textField){
		...
		var width=this.textField.width()+204;//这里可以调整自动完成的宽度
		var minChars=1;
		var currRow=0;
		...
		this.textField.after('<div class="suggestions" style="max-height:400px;min-height:200px;overflow-x:hidden;width:500px;overflow-y:auto;" id="' + this.divId + '"></div>');
		...
		this.show=function(height){
			this.holder.css({
				...
				"top":this.textField.position().top + this.textField.height() + 3+ "px",
				"height":height + "px",
				'z-index':999
			});
